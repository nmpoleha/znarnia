...
/* Развёрнутая карточка «Только октябрь» (14 занятий) — для программ,
   у которых в октябре 14 занятий вместо 12: 1–4, 10, 11 база. */
const OCT_14 = {
  base: '8 200 ₽',
  now: '7 200 ₽',
  baseNote: 'за 12 занятий',
  lessons: 'В октябре будет 14 занятий',
  lessonsDesc: 'По календарю в октябре будет на 2 занятия больше, поэтому к базовой стоимости добавляется:',
  plus: '+ 900 ₽ за 2 дополнительных занятия',
  bonus: ['1 онлайн-урок с учителем', '1 урок-тренажёр'],
  total: '8 100 ₽',
  totalNote: 'за 14 занятий',
          ...
          <div className="so-modal__dialog so-modal__dialog--wide" role="dialog" aria-modal="true" aria-label="Выберите формат обучения" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="so-fmt">
              ...
              <div className="so-fmt__grid">
                ...
                {fmt.oct ? (
                  <article className="so-fmt-card so-fmt-card--oct">
                    <div className="so-fmt-card__top">
                      ...
                      <div className="so-fmt-card__heading">
                        ...
                        <p className="so-fmt-card__desc">{typo('Оплата за один месяц')}</p>
                      </div>
                    </div>
                    <span className="so-fmt-card__rule" aria-hidden="true" />

                    <div className="so-oct__line">
                      <div className="so-oct__line-label">
                        <span className="so-oct__line-title">{typo('Базовая стоимость')}</span>
                        <span className="so-oct__line-note">{typo(fmt.oct.baseNote)}</span>
                      </div>
                      <div className="so-oct__base">
                        <span className="so-oct__old">{fmt.oct.base}</span>
                        <span className="so-oct__arrow" aria-hidden="true">→</span>
                        <span className="so-oct__now">{fmt.oct.now}</span>
                      </div>
                    </div>

                    <div className="so-oct__extra">
                      ...
                      <div className="so-oct__extra-body">
                        ...
                        <div className="so-oct__extra-mid">
                          ...
                          <ul className="so-oct__extra-list">
                            {fmt.oct.bonus.map((b) => (
                              <li key={b}>
                                <span className="so-oct__check" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none"><path d="M6 12.5l3.6 3.6L18 7.6" stroke="#fff" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round"/></svg></span>
                                <span>{typo(b)}</span>
                              </li>
                            ))}
                          </ul>
                        </div>
                        <p className="so-oct__extra-plus">{fmt.oct.plus}</p>
                      </div>
                    </div>

                    <div className="so-oct__line so-oct__line--total">
                      <div className="so-oct__line-label">
                        <span className="so-oct__line-title">{typo('Итого за октябрь')}</span>
                        <span className="so-oct__line-note">{typo(fmt.oct.totalNote)}</span>
                      </div>
                      <div className="so-oct__total">{fmt.oct.total}</div>
                    </div>

                    <div className="so-fmt-card__actions">
                      <a href="#" className="so-btn so-btn--pay">{fmt.oct.payLabel} →</a>
                    </div>
                  </article>
                ) : (
                  <article className="so-fmt-card">
                    <div className="so-fmt-card__top">
                      ...
                      <div className="so-fmt-card__heading">
                        <h4 className="so-fmt-card__title">{typo('Только октябрь')}</h4>
                        <p className="so-fmt-card__desc">{typo('12 уроков по выбранной программе')}</p>
                      </div>
                    </div>
                    <span className="so-fmt-card__rule" aria-hidden="true" />
