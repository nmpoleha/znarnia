...
const PRODUCT = {
  badge: 'Для учеников 1–9 классов и их родителей',
  name: 'Интеллектуальный',
  nameAccent: 'клуб',
  subAccent: 'Бесплатная подписка',
  sub: 'каждую неделю — новые полезные материалы для учёбы и развития мышления.',
}
...
const THESES = [
  {
    ...
    text: 'Интерактивные тренажёры для практики, логики и умения рассуждать.',
  },
  {
    icon: <IconChecklist />,
    title: 'Контроль знаний',
    text: 'Проверочные работы по школьной программе, а для 9 класса — регулярные варианты ОГЭ.',
  },
  {
    icon: <IconCap />,
    title: 'Для родителей',
    text: 'Практические гайды об учёбе, поступлении, дисциплине и самостоятельности.',
  },
]
    ...
    <div className="sh-page">
      ...
      <section className="sh-hero">
        ...
        <div className="sh-hero__bg" aria-hidden="true">
          ...
          <svg className="sh-hero__blob sh-hero__blob--bl" viewBox="0 0 560 240" fill="none" preserveAspectRatio="xMidYMid meet">
            <path d="M70 118C70 58 190 34 300 56c96 20 200-2 224 58 26 66-96 108-224 100-120-8-230-36-230-96Z" fill="#f3eefc" />
            <path d="M120 140c0-42 96-58 182-44 78 12 156 0 168 44 12 46-78 72-176 66-96-6-174-24-174-66Z" fill="#ece5f9" />
          </svg>
        </div>
        ...
        <div className="sh-wrap sh-hero__inner">
          <div className="sh-hero__left">
            <div className="sh-hero__badge">
              <span className="sh-hero__badge-icon" aria-hidden="true">
                <svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 3.2l2.6 5.5 6 .7-4.5 4.1 1.2 5.9L12 16.9 6.7 19.4l1.2-5.9L3.4 9.4l6-.7z"/></svg>
              </span>
              {nb(PRODUCT.badge)}
            </div>

            <h1 className="sh-hero__title">
              {PRODUCT.name}<br /><span className="sh-hero__title-accent">{PRODUCT.nameAccent}</span>
            </h1>
            <p className="sh-hero__sub">
              <span className="sh-hero__sub-accent">{PRODUCT.subAccent}</span>: {nbd(PRODUCT.sub)}
            </p>

            <ul className="sh-hero__theses">
              {THESES.map((t, i) => (
                <li key={i} className="sh-hero__thesis">
                  <span className="sh-hero__thesis-icon" aria-hidden="true">{t.icon}</span>
                  <div className="sh-hero__thesis-body">
                    <div className="sh-hero__thesis-title">{nb(t.title)}</div>
                    <div className="sh-hero__thesis-text">{nbd(t.text)}</div>
                  </div>
                </li>
              ))}
            </ul>
